...
class UnifiedConfigManager:
    ...
    def get_default_model(self) -> str:
        ...
        # 优先返回快速分析模型，保持向后兼容
        return settings.get("quick_analysis_model", settings.get("default_model", "nvidia/nemotron-3-super-120b-a12b"))
    ...
    def get_quick_analysis_model(self) -> str:
        ...
        return settings.get("quick_analysis_model") or settings.get("quick_think_llm", "nvidia/nemotron-3-super-120b-a12b")

    def get_deep_analysis_model(self) -> str:
        """获取深度分析模型"""
        settings = self.get_system_settings()
        # 优先读取新字段名，如果不存在则读取旧字段名（向后兼容）
        return settings.get("deep_analysis_model") or settings.get("deep_think_llm", "nvidia/nemotron-3-super-120b-a12b")
